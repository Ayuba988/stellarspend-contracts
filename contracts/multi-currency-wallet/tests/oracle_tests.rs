...
use multi_currency_wallet::{MultiCurrencyWallet, ConversionRequest};
use shared::oracle::{PriceOracle, Price};
...
fn test_fresh_rate_accepted() {
    ...
    // Add balance
    MultiCurrencyWallet::add_balance(
        env.clone(),
    ...
    let result = MultiCurrencyWallet::convert_currency(
        env.clone(),
        request,
    );
...
fn test_stale_rate_rejected() {
    ...
    let result = std::panic::catch_unwind(|| {
        MultiCurrencyWallet::convert_currency(
            env.clone(),
            request,
        );
    });
